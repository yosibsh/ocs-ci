...
class IBMDeployment(Deployment):
    ...
    def destroy_ocs(self):
        ...
        rbd_pvcs = [
            p
            for p in pvc.get_all_pvcs_in_storageclass("ocs-storagecluster-ceph-rbd")
            if not (
        ...
        cephfs_pvcs = pvc.get_all_pvcs_in_storageclass("ocs-storagecluster-cephfs")
    ...
    def destroy_lso(self):
        ...
        subscription = ocp.OCP(
            kind="Subscription",
            resource_name=constants.LOCAL_STORAGE_SUBSCRIPTION,
            namespace=lso_namespace,
        ).get()
        currentCSV = subscription["status"]["currentCSV"]
        ocp.OCP().exec_oc_cmd(
            f"delete subscription {constants.LOCAL_STORAGE_SUBSCRIPTION} -n {lso_namespace}"
        )
        ocp.OCP().exec_oc_cmd(
