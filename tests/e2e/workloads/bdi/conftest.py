...
def install_helm_factory_fixture(request):
    ...
    # temp_tiller_yaml = tempfile.NamedTemporaryFile(
    ...
    def factory():
        ...
        helm_cmd = (
            f"./helm init --client-only --stable-repo-url https://charts.helm.sh/stable "
        ...
        #     command=f"process -f {tiller_template_url} "
        #     f"-p TILLER_NAMESPACE={tiller_namespace} "
        #     f"-p HELM_VERSION={helm_version}",
        #     out_yaml_format=True,
        # )
        # templating.dump_data_to_temp_yaml(output, temp_tiller_yaml.name)
        # ocp_proj.exec_oc_cmd(command=f"create -f {temp_tiller_yaml.name}")
        # ocp_proj.exec_oc_cmd(
        #     command=f"rollout status deployment {tiller_namespace}",
    ...
    def finalizer():
        ...
        # if os.path.isfile(temp_tiller_yaml.name):
        #     exec_cmd(cmd="rm -f " + temp_tiller_yaml.name)
